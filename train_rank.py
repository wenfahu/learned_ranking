...
def train(gallery_loader, query_loader, model, criterion, optimizer, scheduler, topk):
    ...
    for idx in range(num_episodes):
        ...
        dist = torch.mm(query_f, gallery_f)
        sorted_dist, sorted_idx = torch.sort(dist, dim=1)

        k_nearest_gallery = torch.gather(gallery_f.repeat(query_f.size(0), 1), sorted_idx[:, :topk])
        # k_nearest_gallery of shape QueryBatch x TopK x FeatDim
        k_nearest_gallery_label = torch.gather(gallery_l.repeat(query_f.size(0), 1), sorted_idx[:, :topk])
        ...
        relations_label = query_label == k_nearest_gallery_label
        ...
        optimizer.zero_grad()
        out = model(relations)
        loss = criterion(out, relations_label)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gallery-batch-size', type=int, default='100')
    ...
    gallery_loader = torch.util.data.DataLoader(gallery_set, batch_size = args.gallery_batch_size)
    query_loader = torch.util.data.DataLoader(query_set, batch_size = args.query_batch_size)

    model = ScoreNetwork(args.topk)
    criterion = nn.SmoothL1Los()
    optimizer = torch.optim(model.parameters(), lr=args.lr, amsgrad=True)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.step_size, args.lr_decay)

    for epoch_idx in range(args.num_epoch):
        train(gallery_loader, query_loader, model, criterion, optimizer, scheduler, topk)
